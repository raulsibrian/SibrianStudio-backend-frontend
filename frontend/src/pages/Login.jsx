...
export default function Login() {
  ...
  const iniciarSesion = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('https://sibrianstudio-backend-frontend.onrender.com/api/auth/login', {
        method: 'POST',
  ...
  return (
    <div style={{ maxWidth: '400px', margin: '60px auto', padding: '30px', fontFamily: 'sans-serif', background: '#f9f9f9', borderRadius: '8px', border: '1px solid #ddd', textAlign: 'center' }}>
      ...
      <form onSubmit={iniciarSesion} style={{ display: 'flex', flexDirection: 'column', gap: '15px' }}>
        ...
        <button type="submit" style={{ padding: '12px', background: '#333', color: '#fff', border: 'none', borderRadius: '4px', fontWeight: 'bold', cursor: 'pointer' }}>
          Ingresar
        </button>
      </form>
